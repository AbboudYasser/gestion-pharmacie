...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    const currentUser = PharmacyAPI.currentUser();
...
function adjustUIForUserRole(role) {
    // إخفاء الأقسام غير المناسبة للدور
    const sections = {
...
async function loadDashboardData() {
    try {
        ...
        // التأكد من أن `dashboardStats.data` موجود قبل الوصول إلى `stats`
        ...
        // التأكد من أن `prescriptions.data` موجود قبل الوصول إلى `prescriptions`
        ...
        // التأكد من أن `lowStockItems.data` موجود قبل الوصول إلى `items`
        updateLowStockItems(lowStockItems.data);
        
        // تحميل الأدوية الشائعة (هذه البيانات غير موجودة في الخادم الخلفي المبسط، لذا سنستخدم بيانات وهمية)
